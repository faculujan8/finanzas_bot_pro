...
async def heartbeat_check(bot) -> None:
    ...
    # Check UALA noche: between 22:30 and 23:59
    if hour == 22 and minute >= 30 or hour == 23:
        state = get_uala_reminder_state()
        today = now.date()
        if not (state.get("date") == today and state.get("state") in ["done", "waiting_tonight", "waiting_morning"]):
            await uala_reminder_noche_standalone(bot)

    # Check UALA mañana: between 9:00 and 10:00
    if hour == 9:
        await uala_reminder_manana_standalone(bot)

    # Check vencimientos: at 9:15
...
async def post_init(app) -> None:
    ...
    jq.run_daily(lambda ctx: uala_reminder_noche_standalone(ctx.bot), time=dtime(22, 30, tzinfo=TZ), name="uala_noche")
