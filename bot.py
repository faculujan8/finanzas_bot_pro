...
- "Foodgol..." / "Supermercado" → movimientos_ok, rubro=supermercado, monto NEGATIVO
- "Gourdman..." / "Restaurantes y bares" → movimientos_ok, rubro=comida, monto NEGATIVO
- "Merpago*..." / "Supermercado" → movimientos_ok, rubro=supermercado, monto NEGATIVO
- "Www.bocajuniors..." / "Salud y deportes" → movimientos_ok, rubro=futbol, monto NEGATIVO
- REGLA GENERAL: si el subtítulo es "Restaurantes y bares", "Supermercado", "Transporte", "Salud y deportes" → movimientos_ok con el rubro correspondiente. NUNCA a dudosos si el subtítulo es claro.
...
Formato de respuesta:
{
  ...
  "transferencias_propias": [{"fecha":"DD/MM/YYYY","monto":-30000,"detalle":"Facundo Dardo Lujan","subtitulo":"Transferencia enviada"}],
...
9. SIGNO DE TRANSFERENCIAS PROPIAS: el signo del monto SIEMPRE tiene que coincidir con el subtítulo, no solo con el color que veas — "Transferencia enviada" = monto NEGATIVO (sale plata de UALA) SIEMPRE, "Transferencia recibida" = monto POSITIVO (entra plata a UALA) SIEMPRE. Si el color del monto en la imagen no es claro, guiate por el texto del subtítulo, que es más confiable. Incluí siempre el campo "subtitulo" en transferencias_propias."""
...
def norm_monto(m):
    try:
        # Los montos vienen en dos formatos distintos que tienen que normalizar IGUAL:
        #  - crudos desde Claude (JSON numérico o string simple): -9100.0 / "-9100.0"
        #  - formateados por Sheets al leer la planilla: "-$9,100.00" (coma = separador de miles)
        # La coma NUNCA es separador decimal acá, así que se elimina (no se convierte a punto).
        s = str(m).replace("$", "").replace(",", "").replace(" ", "").strip()
        return str(abs(round(float(s), 2)))
    except:
        return str(m)
...
async def handle_uala_screenshot(image_bytes: bytes, mime: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Process ok_raw ────────────────────────────────────────────────────
    # Force items with clear subtitulos from dudosos to ok
    CLEAR_SUBTITULOS = {
        "restaurantes y bares": "comida",
    ...
    movimientos_dudosos_raw = result.get("movimientos_dudosos", [])
    rescued = []
    still_dudosos = []
    for m in movimientos_dudosos_raw:
        subtitulo = str(m.get("subtitulo","")).lower().strip()
        detalle = str(m.get("detalle","")).strip()
        # Don't rescue person transfers
        is_person = bool(re.match(r'^[A-Za-záéíóúñÁÉÍÓÚÑ,. ]+$', detalle)) and len(detalle.split()) >= 2 and "*" not in detalle
        if subtitulo in CLEAR_SUBTITULOS and not is_person:
            m["rubro"] = CLEAR_SUBTITULOS[subtitulo]
            rescued.append(m)
        else:
            still_dudosos.append(m)
    result["movimientos_dudosos"] = still_dudosos
    result["movimientos_ok"] = result.get("movimientos_ok", []) + rescued
    ...
    # Corrección de signo determinística: el subtítulo (texto) es más confiable que el
    # color/signo que Claude haya podido leer mal en la imagen. Si dicen, forzamos el signo.
    for t in transferencias:
        subtitulo_t = str(t.get("subtitulo", "")).lower()
        try:
            monto_val = float(t.get("monto", 0))
        except (TypeError, ValueError):
            monto_val = 0.0
        if "enviada" in subtitulo_t:
            t["monto"] = -abs(monto_val)
        elif "recibida" in subtitulo_t:
            t["monto"] = abs(monto_val)
        else:
            t["monto"] = monto_val
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    process_as_new = False  # flag: if True, skip context and process as fresh message
        ...
        if pendiente == "detalle":
            ...
            if es_mensaje_nuevo(text):
                ...
                if rubro is None:
                    user_context.pop(chat_id, None)
                    process_as_new = True
                    await update.message.reply_text("❓ No entendí el rubro, lo proceso como mensaje nuevo...")
                else:
            ...
            if txt_lower in YA_ANOTADO_PHRASES or txt_lower in ["no", "n"]:
                await update.message.reply_text(f"⏭️ Omitido — ${abs(monto):,.0f} no se anota")
                ...
                if monto < 0:
                    # Salió plata de UALA hacia la cuenta destino
                    rows_t = [
                    ...
                    direccion = f"UALA → {cuenta_destino.upper()}"
                else:
                    # Entró plata a UALA desde la cuenta destino
                    rows_t = [
                        [fecha,  abs(monto), "traspaso", f"Traspaso desde {cuenta_destino.upper()}", "UALA"],
                        [fecha, -abs(monto), "traspaso", f"Traspaso a UALA", cuenta_destino.upper()],
                    ]
                    direccion = f"{cuenta_destino.upper()} → UALA"
                append_rows("MOVIMIENTOS", rows_t)
                await update.message.reply_text(f"✅ Traspaso: {direccion}")
    ...
    # New message (either fresh or context was cleared)
    if ctx and not process_as_new:
        return  # was handled above, don't reprocess
