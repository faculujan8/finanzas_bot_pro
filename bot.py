...
- "Facundo Dardo Lujan" o "Lujan Facundo Dardo" / "Transferencia enviada" → transferencias_propias, monto NEGATIVO
- "Facundo Dardo Lujan" o "Lujan Facundo Dardo" / "Transferencia recibida" → transferencias_propias, monto POSITIVO
...
async def handle_uala_screenshot(image_bytes: bytes, mime: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lee todos los movimientos existentes y construye índice con 3 claves:
    # 1. fecha+monto+detalle  2. fecha+monto  3. fecha+detalle
    # También actualiza el índice al añadir filas nuevas (evita dupes mismo batch)
    ...
    for row in existing_rows[1:]:
        ...
        try:
            m_norm = str(abs(round(float(str(row[1]).replace("$","").replace(",",".").strip()), 2)))
        except:
            m_norm = str(row[1]).strip()
    ...
    def is_dup(fecha, monto, detalle=""):
        ...
        try:
            mn = str(abs(round(float(str(monto).replace("$","").replace(",",".").strip()), 2)))
        except:
            mn = str(monto)
    ...
    def mark_saved(fecha, monto, detalle=""):
        """Add to index so same-batch duplicates are also caught."""
        fn = normalize_date(str(fecha).strip())
        try:
            mn = str(abs(round(float(str(monto).replace("$","").replace(",",".").strip()), 2)))
        except:
    ...
    for m in ok:
        ...
        monto_m = m.get("monto", "")
        detalle_m = m.get("detalle", "")
        if is_dup(fecha_m, monto_m, detalle_m):
            ok_dupes += 1
        else:
            rows_to_save.append([fecha_m, monto_m, m.get("rubro",""), detalle_m, "UALA"])
            mark_saved(fecha_m, monto_m, detalle_m)
            ok_saved += 1
    ...
    for r in rendimientos:
        ...
        monto_r = r.get("monto", "")
        detalle_r = r.get("detalle", "Rendimiento UALA")
        if is_dup(fecha_r, monto_r, detalle_r):
            rend_dupes += 1
        else:
            rows_to_save.append([fecha_r, monto_r, "rendimientos", detalle_r, "UALA"])
            mark_saved(fecha_r, monto_r, detalle_r)
            rend_saved += 1
    ...
    for d in devoluciones:
        fecha_d = d.get("fecha", today)
        monto_d = abs(float(d.get("monto", 0)))
        detalle_d = d.get("detalle", "Devolución")
        if is_dup(fecha_d, monto_d, detalle_d):
            dev_dupes += 1
        else:
            rows_to_save.append([fecha_d, monto_d, d.get("rubro","reintegro gastos"), detalle_d, "UALA"])
            mark_saved(fecha_d, monto_d, detalle_d)
            dev_saved += 1
