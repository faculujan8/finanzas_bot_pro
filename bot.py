    ...
    try:
        ...
        if mime == "application/pdf":
            ...
            raw = await call_claude(
                SYSTEM_PROMPT,
                f"Hoy es {today}. El siguiente es el texto extraído de un extracto/resumen de tarjeta de crédito. "
...
async def cmd_tenencias(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        rows_pesos  = read_sheet("TENENCIA", "A3:B25")
        rows_usd    = read_sheet("TENENCIA", "E3:F25")
        rows_inv    = read_sheet("TENENCIA", "H3:I25")
        ...
        answer = await call_claude(
            "Sos BOTSTERO. Recibís datos crudos de una planilla de finanzas y devolvés un resumen claro y visual en español rioplatense. "
            "Mostrá: saldo de cada cuenta en pesos, total USD con equivalente en pesos, inversiones si hay, y patrimonio total. "
            "Usá emojis. Formato texto plano, sin markdown. Los montos con formato $X.XXX.XXX",
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        f"/tenencias — ver saldos actuales por cuenta\n\n"
