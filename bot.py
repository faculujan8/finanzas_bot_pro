...
async def handle_uala_screenshot(image_bytes: bytes, mime: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for m in ok_raw:
        ...
        # If it looks like a person name → ALWAYS ask (transfers vary by purpose)
        import re
        is_person = bool(re.match(r'^[A-Za-z ,]+$', detalle)) and len(detalle.split()) >= 2
        if is_person:
            # Never auto-categorize person transfers (except self)
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx:
        ...
        if pendiente == "detalle":
            ...
            import re as _re
            is_person_name = bool(_re.match(r'^[A-Za-z ,]+$', detalle)) and len(detalle.split()) >= 2
            if detalle and rubro not in ["traspaso", "sueldo", "rendimientos", "inversion", "dolares"] and not is_person_name:
...
def main():
    ...
    scheduler.add_job(lambda: app.create_task(uala_reminder_noche_standalone(app.bot)), 'cron', hour=22, minute=30)
    scheduler.add_job(lambda: app.create_task(uala_reminder_manana_standalone(app.bot)), 'cron', hour=9, minute=0)
    scheduler.add_job(lambda: app.create_task(check_vencimientos_standalone(app.bot)), 'cron', hour=9, minute=15)
    scheduler.start()
