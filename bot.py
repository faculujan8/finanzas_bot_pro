...
async def uala_reminder_manana_standalone(bot) -> None:
    ...
    if not CHAT_ID:
        return
    state = get_uala_reminder_state()
    yesterday = (datetime.now(TZ) - __import__('datetime').timedelta(days=1)).date()
    # If last state was waiting_tonight from yesterday → send morning reminder
    if state.get("state") == "waiting_tonight" and state.get("date") == yesterday:
