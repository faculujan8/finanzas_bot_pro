...
- "[Nombre persona]" / "Transferencia enviada" → transferencia a otra persona
- "Facundo Dardo Lujan" / "Transferencia enviada" → transferencia a sí mismo
...
5. Otras transferencias enviadas a personas → movimientos_dudosos para preguntar rubro
        ...
        resp = await client.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content": [
                    {"type": "text", "text": f"Hoy es {today}. FECHA_DESDE={desde_fecha if desde_fecha else today}. Solo procesá movimientos con fecha >= FECHA_DESDE. Memoria de comercios conocidos: {memory_str}\n\nAnalizá esta captura de UALA y clasificá los movimientos."},
...
async def get_last_uala_date() -> str:
    """Get the last processed UALA date from Sheets."""
    try:
        rows = read_sheet("MEMORIA_UALA", "D:D")
...
async def handle_uala_screenshot(image_bytes: bytes, mime: str, update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not desde_fecha:
        desde_fecha = today
    logger.info(f"Processing UALA from date: {desde_fecha}")
