...
{"tipo":"tarjeta","banco":"GALICIA","marca":"VISA","cuenta_pago":"TARJETA_VISA_GALICIA","filas":[{"fecha":"DD/MM/YYYY","monto":-5000,"rubro":"comida","detalle":"Nombre del comercio exacto"},{"tipo":"consulta_rubro","detalle":"RADIO*SOMETHING*AR","monto":-3000,"fecha":"DD/MM/YYYY"}],"mensaje":"💳 Procesando..."}
...
- cuenta_pago = "TARJETA_" + marca + "_" + banco (ej: TARJETA_VISA_GALICIA, TARJETA_MASTERCARD_NACION)
...
2. Monto de cada fila = el monto exacto de esa línea (negativo = gasto)
3. Para cuotas (ej: PAYU*AR*ADIDAS 02/06): extraé solo esa cuota, no el total
4. Ignorá: SALDO ANTERIOR, SU PAGO, SALDO PENDIENTE, IMPUESTO DE SELLOS, PERCEPCION, SUBTOTAL, TOTAL
5. Ignorá también: DEV PER RG, PERCEP.AFIP, PERC IIBB — son impuestos/percepciones
6. Fechas: usá la fecha de cada consumo en DD/MM/YYYY
7. Comercios reconocibles (Netflix, Spotify, Uber, PedidosYa, COMBUSTIBLE, MAPFRE, BOCA) → poné rubro directamente
8. Comercios crípticos (PAYU*AR*, MERPAGO*, RADIO*) → tipo consulta_rubro
    ...
    if tipo == "movimiento":
        ...
        for f in filas:
            if f.get("tipo") == "consulta_rubro":
                ...
                rows_ok.append([f.get("fecha",""), f.get("monto",""), f.get("rubro",""), f.get("detalle",""), f"TARJETA_{cuenta_pago.upper()}"])
        if rows_ok:
            append_rows("MOVIMIENTOS", rows_ok)
        if dudosos:
